...
const MOCK_ADMIN = {
  ...
  displayName: 'Administrador Premium',
};
...
export const loginAdmin = async (email, password) => {
  if (isMock) {
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        if (email === 'admin@barbeariapremium.com' && password === 'admin123') {
          const user = { ...MOCK_ADMIN };
          localStorage.setItem('admin_session', JSON.stringify(user));
          window.dispatchEvent(new Event('admin_auth_changed'));
          resolve(user);
        } else {
          reject(new Error('Credenciais de demonstração incorretas. Use: admin@barbeariapremium.com / admin123'));
        }
      }, 800);
    });
  }
